...
def get_calib_from_file(calib_file):
    ...
    return {'P2': P2.reshape(3, 4),
            'P3': P2.reshape(3, 4),
            'R0': np.zeros((3, 3)),
            'Tr_velo2cam': P2.reshape(3, 4)}

def get_calib_extrinsic_from_file(calib_file, extrinsic_file):
    with open(calib_file) as f:
        lines = f.readlines()
    obj = lines[0].strip().split(' ')[1:]
    P2 = np.array(obj, dtype=np.float32).reshape(3, 4)
    ...
    with open(extrinsic_file, 'r', encoding="utf-8") as f:
        file_data = f.read()                 
        extrinsic_data = yaml.load(file_data,Loader=yaml.FullLoader)
    quaternion = extrinsic_data["transform"]["rotation"]
    quaternion = np.array([quaternion['x'], quaternion['y'], quaternion['z'], quaternion['w']])
    rotation_matrix = quaternion_to_rotation_matrix(quaternion)
    translation = extrinsic_data["transform"]["translation"]
    translation_matrix = np.array([translation['x'], translation['y'], translation['z']])
    
    Tr_velo2cam = np.c_[rotation_matrix, translation_matrix]  
    return {'P2': P2.reshape(3, 4),
            'P3': None,
            'R0': np.identity(3),
...
class Calibration(object):
    def __init__(self, calib_file, extrinsic_file):
        if isinstance(calib_file, str):
            # calib = get_calib_from_file(calib_file)
            calib = get_calib_extrinsic_from_file(calib_file, extrinsic_file)
        else:
            calib = calib_file
...
class Object3d(object):
    def __init__(self, line):
        ...
        self.dim = np.array((float(label[8]), float(label[9]), float(label[10])), dtype=np.float32)
    ...
    def generate_corners3d_lidar(self):
        ...
        corners_3d = corners_3d + np.array(self.pos, dtype=np.float32).reshape(3, 1)
