...
class GUPNet(nn.Module):
    def __init__(self, backbone='dla34', neck='DLAUp', downsample=4, mean_size=None, mean_ref=None, ckpt=None):
        ...
        self.mean_ref = nn.Parameter(torch.tensor(mean_ref, dtype=torch.float32),requires_grad=False)
        ...
        self.ref_2d = nn.Sequential(nn.Conv2d(neck_channels, self.head_conv, kernel_size=3, padding=1, bias=True),
                                     nn.ReLU(inplace=True),
                                     nn.Conv2d(self.head_conv, 2, kernel_size=1, stride=1, padding=0, bias=True))
        ...
        self.ref_3d = nn.Sequential(nn.Conv2d(neck_channels+2+self.cls_num, self.head_conv, kernel_size=3, padding=1, bias=True),
                                     nn.BatchNorm2d(self.head_conv),
                                     nn.ReLU(inplace=True),nn.AdaptiveAvgPool2d(1),
                                     nn.Conv2d(self.head_conv, 2, kernel_size=1, stride=1, padding=0, bias=True))
        ...
        self.fill_fc_weights(self.ref_2d)
        ...
        self.ref_3d.apply(weights_init_xavier)
    ...
    def forward(self, input, coord_ranges,calibs, targets=None, K=50, mode='train'):
        ...
        ret['ref_2d']=self.ref_2d(feats[0])
    ...
    def get_roi_feat_by_mask(self,feats,box2d_maps,ref_2d,inds,mask,calibs,coord_ranges,cls_ids):
        ...
        if num_masked_bin!=0:
            ...
            ref2d_masked = extract_input_from_tensor(ref_2d, inds,mask)
            ...
            ref2d_masked = box2d_height
            '''
            ref2d_masked = ref2d_masked[:, 0]
            ref2d_masked = torch.clamp(ref2d_masked, min=1.0)
            '''
            ...
            ref3d_offset = self.ref_3d(roi_feature_masked)[:,:,0,0]
            ref3d_log_std = ref3d_offset[:, 1]
            ref3d_offset = ref3d_offset[:, 0]
            ref3d = (self.mean_ref[cls_ids[mask].long()] + ref3d_offset)
            
            # depth_geo = ref3d / ref2d_masked.squeeze() * roi_calibs[:,0,0]
            # depth_net_out = self.depth(roi_feature_masked)[:,:,0,0]
            
            depth_geo = size_3d[:, 0] / ref2d_masked.squeeze() * roi_calibs[:,0,0]
            depth_net_out = self.depth(roi_feature_masked)[:,:,0,0]
            depth_geo_log_std = (h3d_log_std.squeeze() + 2 * (roi_calibs[:,0,0].log() - ref2d_masked.log())).unsqueeze(-1)
            ...
            res['h3d_log_std'] = h3d_log_std
            res['ref3d_log_std'] = ref3d_log_std
            res['ref_3d'] = ref3d_offset
        else:
            ...
            res['h3d_log_std'] = torch.zeros([1,24]).to(device_id)
            res['ref3d_log_std'] = torch.zeros([1,1]).to(device_id)
        return res
    ...
    def get_roi_feat(self,feats,inds,mask,ret,calibs,coord_ranges,cls_ids):
        ...
        res = self.get_roi_feat_by_mask(feats,box2d_maps,ret['ref_2d'],inds,mask,calibs,coord_ranges,cls_ids)
        return res
